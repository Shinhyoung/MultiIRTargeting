...
OptiTrack Flex 13 카메라 **2대를 동시에** 운용하여 IR 영상을 실시간으로 처리합니다.
...
### 다중 카메라 동시 처리
- 연결된 OptiTrack Flex 13 카메라 **2대 동시 초기화 및 구동**
- 두 카메라 영상을 하나의 창에 좌(Cam0) / 우(Cam1) 나란히 표시
- 카메라별 독립 호모그래피 상태 관리
...
**마지막 업데이트**: 2026-03-04
**버전**: 3.0.0 (다중 카메라 지원, 합성 창 + 스티칭 뷰, 카메라별 오프셋 좌표 체계)
